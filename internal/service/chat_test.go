...
func TestCreateDialog(t *testing.T) {
	...
	gomock.InOrder(
		testRepo.mockChatR.EXPECT().IsUniqDialog(ctx, tests[1].inputIsUniqDialog.firstUserID,
			tests[1].inputIsUniqDialog.secondUserID).Return(tests[1].outputIsUniqDialog.err),
		...
		testRepo.mockChatR.EXPECT().CreateDialog(ctx, tests[2].inputCreateDialog.userID,
			tests[2].inputCreateDialog.name,
			tests[2].inputCreateDialog.authorIDs).Return(tests[2].outputCreateDialog.dialog, tests[2].outputCreateDialog.err),
		testRepo.mockUserR.EXPECT().AddDialog(ctx, tests[2].inputAddDialogForUserID.dialogID,
			tests[2].inputAddDialogForUserID.userID).Return(tests[2].outputAddDialogForUserID.err),
		testRepo.mockUserR.EXPECT().AddDialog(ctx, tests[2].inputAddDialogForAuthorIDs.dialogID,
			tests[2].inputAddDialogForAuthorIDs.userID).Return(tests[2].outputAddDialogForAuthorIDs.err),
	)
